"use client";
import { Filter, Search, ChevronLeft, ChevronRight, CheckCircle2, Clock } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
        <div>
          <h1 className="text-[28px] font-bold text-slate-900 tracking-tight">System-Wide Appointments</h1>
          <p className="text-slate-500 text-sm mt-1">Manage and audit cross-facility bookings.</p>
        </div>
        <div className="flex items-center space-x-3">
          <button className="flex items-center px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-lg hover:bg-slate-50 font-semibold text-xs tracking-wider transition-colors shadow-sm">
            <Filter size={16} className="mr-2 text-slate-500" /> FILTER
          </button>
          <div className="relative w-full sm:w-64">
            <input 
              type="text" 
              placeholder="Search appointments..." 
              className="pl-9 pr-4 py-2.5 border border-slate-200 rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 w-full transition-all placeholder:text-slate-400"
            />
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:justify-between sm:items-center bg-slate-50/80 gap-4">
                ...
                <tr key={item} className="hover:bg-slate-50/80 transition-colors group">
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      <span className="font-bold text-slate-900 text-[13px]">Fatima Jinnah</span>
                      <span className="text-[11px] text-slate-500 font-medium mt-0.5">ID: P-9912</span>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <div className="flex items-center">
                      <div className="w-8 h-8 rounded-full bg-blue-50 border border-blue-100 mr-3 flex items-center justify-center text-[10px] font-bold text-blue-600">DR</div>
                      <span className="font-bold text-slate-700 text-[13px]">Dr. Ali Raza</span>
                    </div>
                  </td>
                  <td className="px-6 py-5 text-slate-600 font-medium text-[13px]">Aga Khan University Hospital</td>
                  <td className="px-6 py-5">
                    <span className="inline-flex items-center px-2.5 py-1 bg-emerald-50 text-emerald-700 text-[9px] font-bold rounded uppercase tracking-wider border border-emerald-100/50">
                      <CheckCircle2 size={12} className="mr-1.5" /> Confirmed
                    </span>
                  </td>
                </tr>
              ))}
